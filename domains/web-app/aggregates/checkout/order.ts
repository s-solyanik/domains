...
import type {PaymentsOrderType, OrderWithAutoGeneratedId} from "domains/entities/payments.order";
...
class PaymentsOrderAggregate {
    ...
    public data() {
        return this.state.data().pipe(
            switchMap(it => EntityResult.unitGet<PaymentsOrderEntity, OrderWithAutoGeneratedId>(it))
        );
    }
}
